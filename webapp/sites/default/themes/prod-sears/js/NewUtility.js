/*
	...
	NOTE: This file will never contain application-specific code
		Do NOT make changes without discussion with OBU UX CORE FED, reach out to Tim.Ryan@searshc.com or Seth.Thomas@searshc.com
*/

//obsolete - still a couple packed jquery UI files calling this
jQuery.fn.bgIframe = jQuery.fn.bgiframe = jQuery.noop;
...
FED.Template = {
	...
	layerClose: '<div id="{0}" class="{1}">' +
		...
		'<div class="closeWrp"><span class="close">CLOSE</span><span class="closeX">X</span></div>' +
 ...
 * @param {String} s the browser specific regex string to match
 * @returns {Boolean} true if the string is matches the user agent, else false
	...
	var ua = navigator.userAgent.toLowerCase(),
		...
		isIE10 = isIE && uaTest(/msie 10/),	//TODO: need to verify this UA when there is an IE10 version to test
		...
		//This mobile check isn't really full blown since the server side redirects true mobile(phones) to m.sears.com
		isMobile = uaTest(/ipad|ipod/),
		stores = {
			'10151': 'kmart',
			'10153': 'sears',
			'10154': 'kenmore',
    ...
    var addBodyCls = function() {
        var cls = [
		isIE ? 'ie ' + (isIE6 ? 'ie6' : (isIE7 ? 'ie7' : (isIE8 ? 'ie8' : (isIE9 ? 'ie9' : 'ie10')))) :
			isGecko ? ('gecko' + (isGecko3 ? ' gecko3' : '')) : isOpera ? 'opera' : isWebKit ? 'webkit' : ''];
		...
		//look for the global kiosk flag
		if (typeof isKiosk !== 'undefined' && (isKiosk === 'true' || isKiosk == true)) {
			cls.push('kiosk');
		}
	...
	return {
		 ...
		 * @param {String} url the file to be added.  The '.css' extension is not required.
	...
	function() {
		...
		toolTip: function(el, options) {
			...
			if (css.left + tip.outerWidth() >= $.q(document).width()) {
				...
				css.left = offset.left - tip.outerWidth() + $('.shcTip').outerWidth() + 30;				
			}
			//if we're going to overrun the bottom, shift it up
			if (css.top + tip.outerHeight() >= $.q(document).height()) {
				css.top = offset.top - tip.outerHeight() - el.height();
...
$('#openLayer').click(function() {
		...
		layer: function(el, options) {
			...
			if (o.closeable) {
				$('#' + o.id + ' .closeWrp').on('click', o.cbClose || cbClose);
			}
 ...
 * @return jQuery
 */
jQuery.fn.centerOnScreen = jQuery.fn.center;
 ...
 * @return jQuery
*/
jQuery.fn.dynamicPopup = function (options) {
	options = $.extend({
 ...
 * @return jQuery
*/
jQuery.fn.dynamicPopup1 = function (options) {
	options = $.extend({
		filename: 'default.html',
		contentname: '#popup',
		windowWidth: '300px',
 ...
 * @return {jQuery}
*/
    jQuery.q.assign = function ( options ) {
        var defaults = {
            interval: 0,
            namespace: undefined
        };
...
FED.onReady = function () {
	...
	//Removed the checks for DIV/SPAN b/c all browsers move these out of HEAD during render, and they're not caught.
	if (/(qa\.ecom\.)|(localhost)|(dev\.ch3\.)|(pilot\.ch4)/.test(location.host)) {
		if (jQuery('head').children('input').length) {
	...
	jQuery.ajaxPrefilter(function(options, originalOptions, xhr) {
		...
		//For all AJAX calls which are NOT script, add a querystring parm for tracking purposes
		if (options.dataType !== 'script') {
			//options will be a string version of the querystring args, even if passed as an object in initial call.
			//leading ? is appended automatically further downstream.
			options.data = qs + (qs.length ? '&' : '') + 'shcAJAX=1';
		}

		//3rd Party ajax calls get a forced timeout.  Anything with a non-'www' subdomain is considered x-domain here.
		if (options.crossDomain && url.indexOf('shld.net') < 0) {
			options.timeout = 1000 * (typeof shcParams !== 'undefined' ? shcParams.ajaxTimeout3P : 3);
...
//DEAD functions - to be removed
/**
 * @class Removed
 * This is just a placeholder for noting global functions that have been declared obsolete and removed from the codebase.</br />
 * While the function definition may still exist, it has been aliased to $.noop or an 'alert' popup error.<br />
 * Check 'Removed' in the 'Show' dropdown on the right side to display functions.
*/
 ...
 * @removed Use $.cookie(name)
 */
getCookieValue = jQuery.noop;

/**
 * @member Removed
 * @method
 * Update a cookie
 * @removed Use $.cookie(name, value)
 */
setCookie = jQuery.noop;
 ...
 * @removed Use $.cookie(name, value)
 */
setCookieValue = jQuery.noop;

/**
 * @member Removed
 * @method
 * Remove a cookie
 * @removed Use $.cookie(name, null)
 */
Delete_Cookie = jQuery.noop;

// noop assignments above are just to make the docgen work
//
setCookie=Delete_Cookie=setCookieValue=getCookieValue = function() {
	//alert('Use of obsolete cookie functions not allowed. Use $.cookie');
};
 ...
 * @method
 * Trim a string
 * @removed Use {@link Global#rtrim} or {@link String#trim}
*/
trimStr = jQuery.noop;
/**
 * @member Removed
 ...
 * @removed Use {@link Global#rtrim} or {@link String#trim}
*/
TrimString = jQuery.noop;
/**
 * @member Removed
 * @method
 * Right trim a string
 * @removed Use {@link Global#rtrim} or {@link String#rtrim}
*/
trimRight = jQuery.noop;
/**
 * @member Removed
 * @method
 * Left trim a string
 * @removed Use {@link Global#ltrim} or {@link String#ltrim}
*/
trimLeft = jQuery.noop;
/**
 * @member Removed
 * @method
 * Left trim a string
 ...
 * @removed Use {@link Global#replaceAll} or {@link String#replaceAll}
*/
js_ReplaceChar = jQuery.noop;
/**
 * @member Removed
 * @method
 * Remove spaces from a string
 * @removed Use {@link String#replaceAll} method
*/
removeSpaces = jQuery.noop;
/**
 * @member Removed
 * @method
 * Trim a phone string
 * @removed Use {@link Global#trim} or {@link String#trim}
*/
phoneTrim = jQuery.noop;
/**
 * @member Removed
 * @method
 * Trim a string
 ...
 * @member Removed
 * @method
 * Legacy function for showing element
 * @removed Use $.show()
*/
showBox = jQuery.noop;
 ...
 * @member Removed
 * @method
 * Legacy function for hiding element
 * @removed Use $.hide()
*/
hideBox = jQuery.noop;
 ...
 * @removed Use FED.Util#isNumeric
 */
fnIsNumeric = jQuery.noop;
 ...
 * @removed Use FED.Util#isNumeric
 */
isNumeric = jQuery.noop;

//noop assignments above are just to make the docgen work
disObjBlk=trimStr=TrimString=trimAll=trimRight=trimLeft=LTrim=RTrim=js_ReplaceChar=removeSpaces=phoneTrim=fnIsNumeric=isNumeric=setImgSrc=function () {
	alert('disObjBlk,trimStr,TrimString,trimAll,trimRight,trimLeft,LTrim,RTrim,js_ReplaceChar,removeSpaces,phoneTrim,showBox,hideBox,fnIsNumeric,isNumeric,setImgSrc fns are obsolete - use standard functions');
	return null;
};
